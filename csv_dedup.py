...
df = pd.concat([pd.read_csv(currentFile, header = None, index_col = False) for currentFile in glob.iglob(r'c:\users\user\documents\*.csv')], ignore_index = True).drop_duplicates(subset = None, keep = 'last').reset_index(drop = True)
